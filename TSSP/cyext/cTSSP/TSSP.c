...
void PerformSuperpixelSLIC(double* lvec, double* avec, double* bvec, double* kseedsl, double* kseedsa, double* kseedsb, double* kseedsx, double* kseedsy, int width, int height, int numseeds, int* klabels, int STEP, double compactness,double ** MapL,double ** MapA,double ** MapB,double ** Amptitude,double ** Xita,double ** Gray,int radius,int gamma)
{
    ...
    double maxpidx; maxpidx = (double)windows - 1; // idx - index
            ...
            for( y = y1; y < y2; y++ )
			{
				for( x = x1; x < x2; x++ )
				{
					...
					for( idy = 0; idy < windows; idy++ )
                    {
                        for( idx = 0; idx < windows; idx++ )
                        {
                            //printf("at offset (%d, %d), subpoint (%d, %d)\n", idx, idy, x+idx, y+idy);
                            qL=MapL[x+idx][y+idy];
							qA=MapA[x+idx][y+idy];
							qB=MapB[x+idx][y+idy];
							...
							if((idx-radius)*(idx-radius)+(idy-radius)*(idy-radius)<=radius*radius)
                            {
                                ...
                                if (py < 0) {py = 0;} else if (py > maxpidx) {py = maxpidx;}
								px=(idx-radius)*sin(alpha)+(idy-radius)*cos(alpha)+radius;
                                if (px < 0) {px = 0;} else if (px > maxpidx) {px = maxpidx;}
                                //printf("%d %d %d %d\n", (int)floor(px), (int)ceil(px), (int)floor(py), (int)ceil(py));
...
# define PACK_INT32(name) \
    mxArray *M##name = mxCreateNumericMatrix(1, 1, mxINT32_CLASS, mxREAL); \
    ((long *)(M##name->data))[0] = name;

# define PACK_ARR3D_UINT8(name, h, w, c) \
    mxArray *M##name = (mxArray *)mxMalloc(sizeof(mxArray)); \
    M##name->classid = mxUINT8_CLASS; \
    M##name->isreal = mxREAL; \
    M##name->ndims = 3; \
    M##name->shape = (mwSize *)mxMalloc(3 * sizeof(mwSize)); \
    M##name->shape[0] = h; M##name->shape[1] = w; M##name->shape[2] = c; \
    M##name->data = name;

# define PACK_ARR3D_DOUBLE(name, h, w, c) \
    mxArray *M##name = (mxArray *)mxMalloc(sizeof(mxArray)); \
    M##name->classid = mxDOUBLE_CLASS; \
    M##name->isreal = mxREAL; \
    M##name->ndims = 3; \
    M##name->shape = (mwSize *)mxMalloc(3 * sizeof(mwSize)); \
    M##name->shape[0] = h; M##name->shape[1] = w; M##name->shape[2] = c; \
...
TSSP_result_t *TSSPmain(int numsuper, mwSize h, mwSize w,
    ...
    PACK_INT32(numsuper)
    PACK_ARR3D_UINT8(img, h, w, 3)
    PACK_ARR3D_DOUBLE(img_gray, h+2*radius, w+2*radius, 1)
    PACK_ARR3D_DOUBLE(img_lab, h+2*radius, w+2*radius, 3)
    PACK_ARR3D_DOUBLE(magnitude, h, w, 1)
    PACK_ARR3D_DOUBLE(direction, h+2*radius, w+2*radius, 1)
    PACK_INT32(radius)
    PACK_INT32(gamma)
    mxArray *plhs[TSSP_nlhs];
    const mxArray *prhs[TSSP_nrhs] = {Mnumsuper, Mimg, Mimg_lab, Mimg_gray, Mmagnitude, Mdirection, Mradius, Mgamma};
    mexFunction(TSSP_nlhs, plhs, TSSP_nrhs, prhs);
    TSSP_result_t *result = (TSSP_result_t *)mxMalloc(sizeof(TSSP_result_t));
    result->nlabels = *(int *)mxGetData(plhs[1]);
    result->labels = (int *)mxGetData(plhs[0]);
    return result;
}

# undef PACK_INT32
# undef PACK_ARR3D_UINT8
# undef PACK_ARR3D_DOUBLE